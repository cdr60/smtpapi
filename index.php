...
class db
{
	...
	function add_file($idmail,$filename)
	{
		...
		$st->closeCursor(); 
		$sth=NULL;
		//Une fois le fichier stocké en base, inutile de le garder
		if ($result->CR=="0") @unlink($filename);
		return $result;
	}
	
	function get_attached_files_list($idmail)
	{
		$result=new stdclass();
		$result->CR="0";
		$result->MSG="";
		$result->DATA=array();
		$sql="SELECT FILENAME,FILEDATA FROM TBMAILATTACH WHERE IDMAIL=".SqlInteger($idmail,FALSE);
		$sth = $this->execute_query($sql);
		if (is_string($sth))
		{
			$result->CR="-2";
...
class command
{
	function __construct()
	{
		...
		{
			$this->fileattachname = TMPDIR . basename($_FILES['userfile']['name']);
	...
	function envoie_mail_secure()
	{
		$result=new stdclass();
		$result->CR="0";
		$result->MSG="";
		if (count($this->cctb)+count($this->bcctb)==0) 
		{
			$result->CR=-1;
		...
		//Accusé réception :
		if ($this->withar!="N") $mail->ConfirmReadingTo = $f->EMAIL;
		foreach($this->bcctb as $dest)
		{
			$d=MkAddressName($dest);
			$mail->addBCC($d->EMAIL, $d->NAME);
		}
		foreach($this->cctb as $dest)
		{
			$d=MkAddressName($dest);
		...
		foreach ($this->filenamelist as $filename) $mail->addAttachment($filename, basename ($filename));
		$ret=TRUE;
		$ret=@$mail->send();
		if ($ret!=TRUE) 
		{
...
if ($cmd->action=="send")
{
	...
	$r=$data->get_attached_files_list($cmd->idmail);
	if ($r->CR!="0") { echo($r->MSG); http_response_code(406); die(); }
	$cmd->filenamelist=$r->DATA;
	echo("Envoie du mail avec l'id ".$cmd->idmail);
	try { $r=$cmd->envoie_mail_secure(); }
	catch (Exception $e)
	{
		$r->CR="-1";
		$r->MSG=$e->getMessage();
		echo($r->MSG); http_response_code(408); die();
	}
	$r=$data->update_mail($cmd->idmail); 
	if ($r->CR!="0") { echo($r->MSG); http_response_code(408); die(); }
	//Nettoyage des fichiers
	foreach ($cmd->filenamelist as $filename)  {@unlink($filename);}
	echo("<br>Envoi réussi"); 
}
$data->close();
